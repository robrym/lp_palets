...
class Template {
  ...
  public function page_classes($print = true) {
    ...
    if ($this->inc_header) {
      ...
      if ($this->l_header_style == 'modern') {
      ...
      } else if ($this->l_header_style == 'dark') {
        ...
        $this->page_classes .= ' page-header-glass page-header-dark';
      }
    }
  ...
  public function get_photo($id = 0, $retina = false, $class = '') {
    echo '<img' . ($class ? ' class="' . $class . '"' : '') . ' src="' . $this->assets_folder . '/media/photos/photo' . ($id ? $id : rand(1, 20)) . ($retina ? '@2x' : '') . '.jpg" alt="">' . "\n";
  }
